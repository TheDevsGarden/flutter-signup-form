...
class MyCustomFormState extends State<MyCustomForm> {
  ...
  MouF selectedSexe = MouF.m;
  bool _accepterTerms = false;
  late Utilisateur _user; // late = pas de valeur par défaut
  String reponse = "";
        ...
        child: Column(
          ...
          children: [
            ...
            CheckboxListTile(
                ...
                value: _accepterTerms,
                onChanged: (value) {
                  setState(() {
                    _accepterTerms = value ?? false;
                  });
                }),
            Padding(
                ...
                onPressed: () {
                  ...
                  if (_formKey.currentState!.validate()) {
                    if (_accepterTerms) {
                      _user = Utilisateur(
                        _nomController.text,
                        _courrielController.text,
                        _telController.text,
                        //problemes ici, je ne sais pas comment récupérer la valeur de sexe
                        selectedSexe,
                        _accepterTerms,
                      );
                      print(_user);
                      reponse = _user.toString();

                      final snackBar = SnackBar(
                        content: Text(_user.toString()),
                        duration: const Duration(
                            seconds: 4), // Durée d'affichage du Snackbar
            ...
            //problemes ici, il faut afficher uniquement lorsque le formulaire est rempli
            Text(reponse), //n'affiche pas
          ],
        ),
      ),
    );
      ...
      children: <Widget>[
        RadioListTile<MouF>(
          ...
          onChanged: (MouF? value) {
            setState(() {
              character = value;
              // probleme ici, je ne sais pas comment récupérer la valeur de sexe
              selectedSexe = value ?? MouF.m;
            });
          },
        ),
        RadioListTile<MouF>(
...
// il faut récupérer la valeur de sexe dans le radio button
  Utilisateur(
      this.nom, this.courriel, this.phone, this.selectedSexe, this.acceptTerms);
  //Utilisateur(this.nom, this.courriel, this.phone, this.acceptTerms);

  @override
  String toString() {
    return "Usager | nom $nom, courriel: $courriel telephone: $phone sexe: $selectedSexe accept $acceptTerms été créé";
    //return "Usager | nom $nom, courriel: $courriel telephone: $phone accept: $acceptTerms été créé";
  }
}
